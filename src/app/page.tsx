  ...
  return (
    <div className="bg-[#FDFBF7]">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute -top-24 -right-24 h-[520px] w-[520px] rounded-full bg-[#C9A96A]/10 blur-3xl" />
        <div className="absolute -bottom-32 -left-32 h-[480px] w-[480px] rounded-full bg-[#E8DCC8]/40 blur-3xl" />
        <div className="relative mx-auto max-w-6xl px-4 py-10 md:py-16 grid md:grid-cols-[1.05fr_0.95fr] gap-8 md:gap-10 items-center">
          <div className="space-y-5">
            <Badge className="bg-white">AI-KOS • EST. 2026 — Luxury Living</Badge>
            ...
            <div className="flex flex-wrap gap-3 pt-1">
              <Link href="/kos/cari"><Button size="lg" className="rounded-full">Cari Kos Sekarang</Button></Link>
          ...
          {/* Mockup Device — Startup/SaaS */}
          <div className="relative">
            <div className="relative mx-auto max-w-[420px] rounded-[28px] border border-[#EDE6D6] bg-white p-3 shadow-soft-lg">
              ...
              {/* floating cards */}
              <div className="absolute -left-4 top-10 hidden md:flex items-center gap-2 rounded-full border border-[#EDE6D6] bg-white px-3 py-2 shadow-soft text-xs">
      ...
      {/* Kos Featured — card luxury soft minimal */}
      <section className="mx-auto max-w-6xl px-4 py-10">
        <div className="flex items-end justify-between gap-4 mb-6">
          <div>
            ...
            <p className="text-sm text-[#8A7D6B]">Hunian terverifikasi • Foto realistic • Shadow lembut modern UI</p>
          </div>
          ...
          {dummyKos.map((k) => (
            <Link key={k.slug} href={`/kos/${k.slug}`} className="group">
              <div className="overflow-hidden rounded-[20px] border border-[#EDE6D6] bg-white shadow-soft group-hover:shadow-soft-lg transition-all">
                ...
                <div className="p-5">
                  <h3 className="font-serif text-[16px] font-semibold text-[#1C1610]">{k.nama}</h3>
                  <p className="mt-0.5 text-xs text-[#8A7D6B] flex items-center gap-1"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="h-3.5 w-3.5"><path d="M12 21s7-5.5 7-11a7 7 0 1 0-14 0c0 5.5 7 11 7 11z"/><circle cx="12" cy="10" r="2.5"/></svg>{k.alamat}</p>
                  <div className="mt-4 flex items-center justify-between">
                    <span className="font-serif text-[18px] font-bold text-[#1C1610]">Rp {k.harga.toLocaleString("id-ID")}<span className="font-sans text-xs font-normal text-[#8A7D6B]"> / bulan</span></span>
                    <span className="rounded-full bg-[#1C1610] px-4 py-1.5 text-xs font-medium text-white group-hover:bg-[#C9A96A] transition-colors">Lihat</span>
                  </div>
                </div>
              </div>
            </Link>
      ...
      <section className="bg-white border-y border-[#EDE6D6]">
        <div className="mx-auto max-w-6xl px-4 py-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={l} className="rounded-2xl border border-[#EDE6D6] bg-[#FDFBF7] p-5 text-center shadow-soft">
      ...
      <section className="mx-auto max-w-6xl px-4 py-10">
        ...
        <ol className="mt-6 grid md:grid-cols-6 gap-3">
          {[
            ...
            <li key={String(label)} className="rounded-2xl border border-[#EDE6D6] bg-white p-4 text-center shadow-soft">
      ...
      <section className="mx-auto max-w-6xl px-4 pb-10">
        <div className="rounded-[24px] bg-gradient-to-br from-[#1C1610] to-[#2C2416] p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-4 shadow-soft-lg border border-[#3A2E1E]">
          ...
          <div className="flex gap-2">
            <Link href="/register"><span className="inline-flex h-10 items-center rounded-full bg-[#C9A96A] px-6 text-sm font-semibold text-white hover:bg-[#B8944F]">Mulai Gratis</span></Link>
            <Link href="/ai" className="inline-flex h-10 items-center rounded-full border border-white/20 bg-white/10 px-6 text-sm font-medium text-white hover:bg-white/15">Tanya AI Concierge</Link>
          </div>
        </div>
      </section>
    </div>
